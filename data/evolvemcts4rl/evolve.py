...
SFT_SYS_PRMPT="You are a math expert who excels at solving mathematical problems step by step."
...
class Nde:
    def __init__(self, solution_prefix: str, parent: Optional['Nde'] = None):
        ...
        self.correctBranches: List[str] = []  # List of correct branches
        self.children: List['Nde'] = []  # List of child states

    def addCorrectBranch(self, branch: str):
        self.correctBranches.append(branch)
...
"""
    ...
    def collectSolutionPrefixes(self) -> List[Dict[str, Any]]:
        ...
        for node in self.mct.nodes:
            ...
            collected_data.append({
                ...
                "reward": rwd,
                "correctBranches": node.correctBranches,
                "falseBranches": node.falseBranches
            })
    ...
    def doConvert(self, evolv_flat_file):
        dpo_coll_data = []
        sft_coll_data =[]
        with open(evolv_flat_file, 'r', encoding='utf-8-sig') as infile:
            for line in infile:
                qobj = json.loads(line)
                q = qobj["question"]
                correctLs = []
                falseLs = []
                for solution in qobj["reasoning_steps"]:
                    correctLs.extend(solution["correctBranches"])
                    falseLs.extend(solution["falseBranches"])
                if correctLs:
                    for elm in correctLs:
                        sft_coll_data.append({
                              "messages": [
                                {
                                  "role": "system",
                                  "content": SFT_SYS_PRMPT
                                },
                                {
                                  "role": "user",
                                  "content": q
                ...
                minlen = min(len(correctLs), len(falseLs))
                if 0 == minlen:
                    continue
                for i in range(minlen):
                    pref = correctLs[i]
                    rej = falseLs[i]
                    dpo_coll_data.append({
                          "input": {
                            "messages": [
                              {
                                "role": "system",
        ...
        if sft_coll_data:
            sft_path = f"{evolv_flat_file.replace('.jsonl', '').replace('.json', '')}.sft.jsonl"
            with open(sft_path, "a") as fd:
                for sftline in sft_coll_data:
                    line = json.dumps(sftline) #json.dumps(list_q, indent=2, ensure_ascii=False)
                    fd.write(f"{line}\n")
        if dpo_coll_data:
            dpo_path = f"{evolv_flat_file.replace('.jsonl', '').replace('.json', '')}.dpo.jsonl"
            with open(dpo_path, "a") as fd:
                for dpoline in dpo_coll_data:
                    line = json.dumps(dpoline) #json.dumps(list_q, indent=2, ensure_ascii=False)
                    fd.write(f"{line}\n")
    ...
    def runQuestionOnly(self):
        ...
        if self.saveAsTree:
            output_file = f"{self.seed_data.replace('.jsonl', '').replace('.json', '')}.%s.tree.json" % uid
        else:
            output_file = f"{self.seed_data.replace('.jsonl', '').replace('.json', '')}.%s.flat.json" % uid
        processed_count = 0
        ...
        if not self.saveAsTree:
            self.doConvert(output_file)
        # Log summary
        logger.info(
